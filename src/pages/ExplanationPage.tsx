  ...
  return (
    <main className="max-w-[880px] mx-auto px-6 pt-12 pb-20">
      ...
      <section className="flex gap-1 mb-14 flex-wrap">
        {loadingSteps
          ? Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="flex-1 min-w-[150px] h-28 rounded-xl" />)
          : flowSteps?.map((s) => (
              <div key={s.id} className="flex-1 min-w-[150px] p-5 rounded-xl border border-border bg-card/50">
      ...
      <section className="mb-14">
        <p className="font-sans text-[11px] tracking-[0.15em] uppercase text-muted-foreground mb-5">Calendário</p>
            ...
            : calendarEvents?.map((ev) => (
                <div
                  ...
                  className={`grid grid-cols-[90px_1fr_auto] gap-5 items-center px-6 py-4 rounded-xl border transition-colors ${
                    ev.is_active ? "bg-primary/5 border-primary/20" : "bg-transparent border-border/30"
                  }`}
                >
                  <div>
                    <div className={`font-sans text-sm font-bold ${ev.is_active ? "text-foreground" : "text-muted-foreground"}`}>{ev.date}</div>
                    <div className="font-sans text-[10px] text-muted-foreground mt-0.5">{ev.day}</div>
                  </div>
                  <div>
                    <div className={`text-sm font-medium mb-0.5 ${ev.is_active ? "text-foreground" : "text-muted-foreground"}`}>{ev.title}</div>
      ...
      <section className="p-7 rounded-2xl border border-border bg-card/30 mb-12">
        <p className="font-sans text-[11px] tracking-[0.15em] uppercase text-muted-foreground mb-4">Regras importantes</p>
      ...
      <div className="flex justify-center">
        <Button onClick={onStart} size="lg" className="px-12 py-6 text-[15px] rounded-xl">Começar agora</Button>
      </div>
    </main>
  );
}
